...
-- Analytics: daily page view counts per path (UTC day)
...
-- Analytics: raw page-view events (country/device/OS/time; see SUPABASE_MIGRATION-ANALYTICS-EVENTS.sql)
...
CREATE INDEX IF NOT EXISTS idx_analytics_events_country ON analytics_page_view_events (country);
...
SELECT jsonb_build_object(
  ...
  COALESCE(
    (
      SELECT jsonb_agg(
        jsonb_build_object('day', agg.d::text, 'views', agg.cnt) ORDER BY agg.d
      )
      FROM (
  ...
  COALESCE(
    (
      SELECT jsonb_agg(
        jsonb_build_object('label', agg.country, 'views', agg.cnt) ORDER BY agg.cnt DESC NULLS LAST
      )
      FROM (
  ...
  COALESCE(
    (
      SELECT jsonb_agg(
        jsonb_build_object('label', agg.device_type, 'views', agg.cnt) ORDER BY agg.cnt DESC NULLS LAST
      )
      FROM (
  ...
  COALESCE(
    (
      SELECT jsonb_agg(
        jsonb_build_object('label', agg.os_name, 'views', agg.cnt) ORDER BY agg.cnt DESC NULLS LAST
      )
      FROM (
